---
name: "narratible-test-runner"
description: "Run narratible verification checks for backend and frontend changes. Use for test requests, regression checks, and pre-PR validation when full automated tests are not yet available."
argument-hint: "Optional scope: backend, frontend, or full"
user-invocable: true
---

# narratible Test Runner
...
- Full scope example:
	- `pwsh -File .github/skills/narratible-test-runner/scripts/run-test-validation.ps1 -Scope full`
- Backend scope example:
	- `pwsh -File .github/skills/narratible-test-runner/scripts/run-test-validation.ps1 -Scope backend`
...
- Example invocation after mutation is executed and observed failing:
	- `pwsh -File .github/skills/narratible-test-runner/scripts/run-test-validation.ps1 -Scope full -MutationTarget "backend/tests/test_cleaner.py::test_regex_cleanup" -MutationSummary "Inverted expected cleanup string" -MutationKilled`
